...
# Carrega o dataset
df <- read.csv("gym/gym.csv", stringsAsFactors = FALSE) %>% mutate(Workout_Type = as.factor(Workout_Type))
...
# Data analysis
p1 <- ggplot(df_exp, aes(x=as.factor(y), y=BMI)) +
  geom_boxplot()

p2 <- ggplot(df_exp, aes(x=as.factor(y), y=BMI)) +
  geom_boxplot()
