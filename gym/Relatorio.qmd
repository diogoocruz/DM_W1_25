...
scatter_plot(df, df$Water_Intake..liters., df$Fat_Percentage, "Ingestão de Água diária (L)", "Percentagem de Gordura")
...
Através de testes ANOVA conseguimos ver que as variáveis que são significativas para a clusterização são `Weight` ,
...
# Exibindo as tabelas para cada variável significativa com kable
library(knitr)
lapply(resultados_tukey_all, function(tabela) {
  var_name <- names(tabela)[2]  # Nome da variável será a segunda coluna da tabela
  kable(tabela, caption = paste("Resultados do Teste de Tukey para a variável", var_name), 
        digits = c(0, 2, 0, 3), 
        format = "html", 
        align = "lccc", 
    ...
    column_spec(2, bold = TRUE, color = "white", background = "darkgray") %>%
...
```{r}
boxplot(df$Weight..kg. ~df$kmeanscluster)
boxplot(df$Height..m. ~df$kmeanscluster)
boxplot(df$Session_Duration..hours. ~df$kmeanscluster)
boxplot(df$Calories_Burned ~df$kmeanscluster)
boxplot(df$Water_Intake..liters. ~df$kmeanscluster)
boxplot(df$Workout_Frequency..days.week. ~df$kmeanscluster)
boxplot(df$BMI ~df$kmeanscluster)
```
